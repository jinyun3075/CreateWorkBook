...
<body>
		...
		if(session.getAttribute("userID")!=null){
			...
			%>
			 ...
			 <a href="work1View.jsp?work1id=<%=value %>&&makeuser=<%=makeUser %>" class="btn btn-primary">처음으로</a>&nbsp;
			<% 	
			}else{
			%>
			<p><%=list.get(p).getWork2_Qw()%><br/></p>
			<form action="startwork.jsp" method="post">
				<input type="hidden" name="val" value="<%=list.get(p).getWork2_value()%>">
				<input type="hidden" name="num" value="<%=p%>">
				<input type="hidden" name="work1id" value="<%=value%>">
				<input type="hidden" name="makeuser" value="<%=makeUser%>">
				<input type="radio" name="qs" value="1"> <%=list.get(p).getWork2_view1()%><br>
				<input type="radio" name="qs" value="2"> <%=list.get(p).getWork2_view2()%><br>
				<input type="radio" name="qs" value="3"> <%=list.get(p).getWork2_view3()%><br>
				<input type="radio" name="qs" value="4"> <%=list.get(p).getWork2_view4()%><br>
