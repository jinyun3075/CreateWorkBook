	...
	<table class="table table-striped"
				...
				<tbody>
					<%
						...
						ArrayList<Work1DTO> list= DAO.publicgetlist();
						for(int i=0;i<list.size();i++){
					%>
					<tr>
						<td><%=list.get(i).getWork1Id() %></td>
						<td><a href="work1View.jsp?title=<%=list.get(i).getWork1Title()%>&&work1id=<%=list.get(i).getWork1Id() %>&&makeuser=<%=list.get(i).getUserId()%>"><%= list.get(i).getWork1Title().replaceAll(" ", "&nbsp").replaceAll(">", "&gt;").replaceAll("<", "&lt;").replaceAll("\n","<br>") %></a></td>
